...
<template>
    <form class="w-full flex items-center justify-between gap-5 flex-col md:flex-row" @submit.prevent="onSubmit">
        <input type="text"
            ...
            :value="city" @input="emit('update:city', $event.target.value)" placeholder="Введите город" />
        <button type="submit" :disabled="error"
            class="w-full md:w-[35%] px-2 py-3 bg-emerald-500 rounded-2xl hover:bg-emerald-700 transition-colors duration-300 cursor-pointer disabled:opacity-60">Получить
            данные</button>
    </form>
</template>